...
const screenshotEmbed = {
  ...
  description: 'Take a look at the sleek, modern interface:',
  color: 0x1976D2,
  image: {
    url: process.env.SCREENSHOT_URL || 'https://via.placeholder.com/800x600.png?text=Add+your+screenshot+URL'
  }
};
  ...
  const req = https.request(options, (res) => {
    ...
    res.on('end', () => {
      if (res.statusCode === 204 || res.statusCode === 200) {
        ...
        if (process.env.SCREENSHOT_URL) {
          console.log('📸 Screenshot included from:', process.env.SCREENSHOT_URL);
        } else {
          console.log('💡 Tip: Set SCREENSHOT_URL environment variable to include launcher screenshots');
        }
      } else {
